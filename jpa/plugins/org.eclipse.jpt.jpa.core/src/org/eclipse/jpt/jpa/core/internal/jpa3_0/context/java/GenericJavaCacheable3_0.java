...
import org.eclipse.jpt.jpa.core.jpa3_0.resource.java.CacheableAnnotation3_0;

/**
 * Java cacheable
 */
public class GenericJavaCacheable3_0 extends AbstractJavaContextModel<JavaCacheableReference3_0>
		implements Cacheable3_0 {
	protected Boolean specifiedCacheable;
	protected boolean defaultCacheable;

	public GenericJavaCacheable3_0(JavaCacheableReference3_0 parent) {
		super(parent);
	...
	public void setSpecifiedCacheable(Boolean cacheable) {
		if (ObjectTools.notEquals(cacheable, this.specifiedCacheable)) {
			if (cacheable == null) {
				...
				CacheableAnnotation3_0 annotation = this.getCacheableAnnotationForUpdate();
	...
	private Boolean buildSpecifiedCacheable() {
		CacheableAnnotation3_0 annotation = this.getCacheableAnnotation();
		if (annotation == null) {
			return null;
		}
		Boolean value = annotation.getValue();
	 ...
	 */
	protected CacheableAnnotation3_0 getCacheableAnnotation() {
		return (CacheableAnnotation3_0) this.getJavaResourceType().getAnnotation(this.getCacheableAnnotationName());
	}

	/**
	 * Build the annotation if it does not exist.
	 */
	protected CacheableAnnotation3_0 getCacheableAnnotationForUpdate() {
		CacheableAnnotation3_0 annotation = this.getCacheableAnnotation();
		return (annotation != null) ? annotation : this.buildCacheableAnnotation();
	}

	protected CacheableAnnotation3_0 buildCacheableAnnotation() {
		return (CacheableAnnotation3_0) this.getJavaResourceType().addAnnotation(this.getCacheableAnnotationName());
	}
	...
	protected String getCacheableAnnotationName() {
		return CacheableAnnotation3_0.ANNOTATION_NAME;
	}

	// ********** misc **********

	protected JavaCacheableReference3_0 getCacheableReference() {
		return this.parent;
	}
