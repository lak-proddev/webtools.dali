...
import org.eclipse.jpt.jpa.core.internal.jpa3_0.context.SimpleMetamodelField3_0;
 ...
 * <p>
 * The mapping annotation is <code>null</code> for default mappings. It will be
 * faulted into existence whenever {@link #getAnnotationForUpdate()} is called.
 * It will <em>not</em> return to <code>null</code> automatically when all its
 * state is defaulted; it must be explicitly cleared via
 * {@link JavaSpecifiedPersistentAttribute#setMappingKey(String)}.
 */
public abstract class AbstractJavaAttributeMapping3_0<A extends Annotation>
		extends AbstractJavaContextModel<JavaSpecifiedPersistentAttribute>
		implements JavaAttributeMapping, AttributeMapping3_0 {
	protected boolean default_;

	protected AbstractJavaAttributeMapping3_0(JavaSpecifiedPersistentAttribute parent) {
		super(parent);
	...
	/**
	 * This method should only be called on mappings that can occur by default (e.g.
	 * <code>Basic</code>, <code>Embedded</code>, <code>OneToOne</code>, and
	 * <code>OneToMany</code>).
	 */
	...
	public Iterable<String> getAllOverridableAttributeMappingNames() {
		return this.isOverridableAttributeMapping() ? new SingleElementIterable<String>(this.getName())
				: EmptyIterable.<String>instance();
	}

	public Iterable<String> getAllOverridableAssociationMappingNames() {
		return this.isOverridableAssociationMapping() ? new SingleElementIterable<String>(this.getName())
				: EmptyIterable.<String>instance();
	}
	...
	public MetamodelField3_0 getMetamodelField() {
		return new SimpleMetamodelField3_0(this.getMetamodelFieldModifiers(), this.getMetamodelFieldTypeName(),
				this.getMetamodelFieldTypeArgumentNames(), this.getMetamodelFieldName());
	}
	...
	protected String getMetamodelFieldTypeName() {
		return JPA3_0.SINGULAR_ATTRIBUTE;
	}
	...
	/**
	 * By default, we add only the mapping's attribute type name; but collection
	 * relationship mappings will also need to add the key type name if the
	 * "collection" is of type {@link java.util.Map}.
	 */
	...
	public String getMetamodelTypeName() {
		return ((SpecifiedPersistentAttribute3_0) this.getPersistentAttribute()).getMetamodelTypeName();
	}
		...
		if (!this.getTypeMapping().attributeMappingKeyAllowed(this.getKey())) {
			if (this.getPersistentAttribute().isVirtual()) {
				messages.add(this.buildValidationMessage(this.getVirtualPersistentAttributeTextRange(),
						JptJpaCoreValidationMessages.PERSISTENT_ATTRIBUTE_INVALID_MAPPING, // TODO KFB - different
																							// message for virtual
																							// attribute
						this.getName()));
			} else {
				messages.add(this.buildValidationMessage(this.getValidationTextRange(),
						JptJpaCoreValidationMessages.PERSISTENT_ATTRIBUTE_INVALID_MAPPING, this.getName()));
			}
		}
	}
